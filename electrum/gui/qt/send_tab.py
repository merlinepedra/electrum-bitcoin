...
class SendTab(QWidget, MessageBoxMixin, Logger):
    ...
    def update_fields(self, pi):
        recipient, amount, description, amount_required, validated = pi.get_fields_for_GUI(self.wallet)
        if recipient:
        ...
        if validated is not None:
            self.payto_e.setGreen() if validated else self.payto_e.setExpired()
    ...
    def read_invoice(self) -> Optional[Invoice]:
        ...
        try:
            invoice = self.payment_identifier.get_invoice(self.wallet, amount_sat)
    ...
    def on_round_2(self, pi):
        ...
        if pi.error:
            ...
            return
        self.update_fields(pi)
        invoice = pi.get_invoice(self.wallet, self.get_amount())
    ...
    def do_pay_or_get_invoice(self):
        ...
        if pi.needs_round_2():
            amount = self.get_amount()
            coro = pi.round_2(self.round_2_signal.emit, amount)
